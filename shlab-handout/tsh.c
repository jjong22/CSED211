...
void eval(char *cmdline) 
{
    ...
    if (argv[0] == NULL)
        return; // No command entered, just end the function

    is_built_in = builtin_cmd(argv); // 1 if built-in command, 0 if not built-in command
    if (!is_built_in){
        ...
        if (pid == 0){
            // Child process
            // Unblock SIGCHLD
            procmask_result = sigprocmask(SIG_UNBLOCK, &mask, NULL); // before run external command, need to unblock SIGCHLD
            
            setpgid(0, 0); // Seperate process group 
            execve_result = execve(argv[0], argv, environ); // Run external command
            ...
            if (is_background)
                printf("[%d] (%d) %s", pid2jid(pid), pid, cmdline); // Print state of background
            else
...
void do_bgfg(char **argv) 
{
    ...
    // Check if the argv[1] is job id
    is_jid = argv1[0] == '%';
    for(i = 1; argv1[i] != '\0' && argv1[i] != ' '; i++) 
        is_jid = is_jid && '0' <= argv1[i] && argv1[i] <= '9';
        // arg[1][0] = '%' and all other is number, is_jid is true.

    // Check if the argv[1] is pid
    for (i = 0; argv1[i] != '\0' && argv1[i] != ' '; i++)
        is_pid = is_pid && '0' <= argv1[i] && argv1[i] <= '9';
        // if arg[1] is all number, is_pid is true.


    // The argv[1] is job id
    if (is_jid) {
        jid = atoi(argv1 + 1);
        job = getjobjid(jobs, jid); // Get job using jid
    ...
    // The argv[1] is pid
        ...
        if (job == NULL){
            ...
            jid = job->jid; // Get job id of job
    }
    
    // The argv[1] is not jid nor pid
    else{
...
void waitfg(pid_t pid)
{
    ...
    // Wait until the job is not running on foreground
...
void sigchld_handler(int sig) 
{
    ...
    while ((pid = waitpid(-1, &status, WNOHANG|WUNTRACED)) > 0) {
        ...
        if (WIFSTOPPED(status)) {
            ...
            jid = pid2jid(pid); // Get job id
            
            printf("Job [%d] (%d) Stopped by signal %d\n", jid, pid, WSTOPSIG(status)); // Print log messsage
        }
        ...
        // Process is terminated (invalid)
            ...
            printf("Job [%d] (%d) terminated by signal %d\n", jid, pid, WTERMSIG(status)); // Print log messsage
...
void sigint_handler(int sig) 
{
    ...
    // Kill process
    kill_result = kill(-pid, sig); // use -pid
